...
> Total tools: 241 (excluding hidden)
...
Conexiones con plataformas e-commerce (Shopify, WooCommerce, PrestaShop, VTEX) y servicios externos.

> **Note:** WAzion supports multiple e-commerce platforms: **Shopify**, **WooCommerce**, **PrestaShop**, and **VTEX**. While the tool names reference Shopify for historical reasons, the underlying integration system works with all supported platforms. Use the same tools regardless of your platform.

| Tool | Type | Description | Key Parameters |
|------|------|-------------|----------------|
| `connect_shopify` | mutation | Connects your e-commerce store (Shopify, WooCommerce, PrestaShop, or VTEX) to search products, orders, and customers | shopify_url, shopify_token |
| `test_shopify_connection` | query | Verifies that the connection with your e-commerce platform works correctly | shopify_url, shopify_token |
| `verify_shopify_write_permission` | query | Verifies if the platform connection has the write_products permission needed | -- |
| `configure_shopify_locales` | mutation | Configures how languages and URLs are handled for your store | platform_connected, shopify_locale_mode, shopify_fixed_locale, shopify_content_locale_mode, +2 more |
| `disconnect_shopify` | mutation | Disconnects the e-commerce platform integration | platform_connected, platform_url, platform_token |
...
| `publish_question_as_faq` | mutation | Publica manualmente una pregunta del historial como FAQ en tu tienda online | question_id |
...
| `get_shopify_sync_status` | query | Estado de conexion e-commerce y estadisticas de clientes CRM sincronizados | -- |
...
| Tool | Type | Description | Key Parameters |
|------|------|-------------|----------------|
| `search_products` | query | Busca productos en tu tienda online (Shopify, WooCommerce, PrestaShop, VTEX) o en el CRM configurado | query, limit |
